...
const BESTIARY = [
  ...
  },
  {
    ...
    speed: '30 фт.',
    stats: [
      { name: 'СИЛ', val: 16, mod: '+3', save: '+6' },
      { name: 'ЛОВ', val: 17, mod: '+3', save: '+3' },
      { name: 'ТЕЛ', val: 18, mod: '+4', save: '+7' },
      { name: 'ИНТ', val: 12, mod: '+1', save: '+1' },
      { name: 'МДР', val: 14, mod: '+2', save: '+5' },
    ...
    senses: 'Тёмное зрение 120 фт. (сквозь магическую Тьму), пассивное Восприятие 18',
    languages: 'Инфернальный, Телепатия 120 фт.',
    treasures: '',
    traits: [
      {
        name: 'Шипастая шкура',
        desc: 'В начале каждого своего хода шипастый дьявол причиняет 5 (1к10) колющего урона всем схватившим его существам.',
      },
      {
        name: 'Дьявольское зрение',
        desc: 'Магическая тьма не мешает тёмному зрению дьявола.',
      },
      {
        name: 'Сопротивление магии',
        desc: 'Дьявол совершает с преимуществом спасброски от заклинаний и прочих магических эффектов.',
      },
    ],
    actions: [
      {
        name: 'Мультиатака',
        desc: 'Дьявол совершает три рукопашные атаки: одну хвостом и две когтями. Альтернативно — дважды использует Метание пламени.',
      },
      {
        name: 'Коготь',
        desc: 'Рукопашная атака оружием: +6 к попаданию, досягаемость 5 фт., одна цель. Попадание: 6 (1к6 + 3) колющего урона.',
      },
      {
        name: 'Хвост',
        desc: 'Рукопашная атака оружием: +6 к попаданию, досягаемость 5 фт., одна цель. Попадание: 10 (2к6 + 3) колющего урона.',
      },
      {
        name: 'Метание пламени',
        desc: 'Дальнобойная атака заклинанием: +5 к попаданию, дистанция 150 фт., одна цель. Попадание: 10 (3к6) урона огнём. Если цель — горючий предмет, которого никто не несёт и не носит, она загорается.',
      },
    ],
...
const BeastPage = ({ beast }: { beast: Beast }) => (
  <div className="w-full h-full flex flex-col md:flex-row overflow-hidden">
    ...
    <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4 text-sm">
      ...
      {/* Описание */}
      {'description' in beast && beast.description && (
        <div>
          <p className="text-[10px] uppercase tracking-widest text-gold/50 font-display mb-2 border-t border-gold/10 pt-3">Описание</p>
          <p className="font-body text-xs text-foreground/70 leading-relaxed italic">{beast.description}</p>
        </div>
      )}
    </div>
  </div>
      ...
      >
        ...
        <div className="flex flex-col md:flex-row overflow-hidden" style={{ maxHeight: '90vh' }}>
          ...
          <div className="flex-1 overflow-y-auto px-8 py-7 space-y-5">
            ...
            {'description' in beast && beast.description && (
              <div>
                <p className="text-[10px] uppercase tracking-widest text-gold/50 font-display mb-2 border-t border-gold/10 pt-4">Описание</p>
                <p className="font-body text-sm text-foreground/70 leading-relaxed italic">{beast.description}</p>
              </div>
            )}
          </div>
        </div>
