...
const BESTIARY_CHAPTERS = [
  ...
  },
  {
    ...
    habitat: 'Девять Преисподних',
    cr: '3', xp: '700', profBonus: '+2',
    ac: 13, initiative: '+2', hp: '58 (9d8 + 18)', speed: '30 фт.',
    stats: [
    ...
    treasures: 'Оружейные',
    traits: [
      { name: 'Сопротивление магии', desc: 'Бородатый дьявол совершает с Преимуществом спасброски от заклинаний и прочих магических эффектов.' },
    ],
    actions: [
      { name: 'Мультиатака', desc: 'Дьявол совершает одну атаку Бородой и одну атаку Дьявольской глефой.' },
      { name: 'Борода', desc: 'Бросок рукопашной атаки: +5, досягаемость 5 фт. Попадание: 7 (1к8 + 3) Колющего урона, и цель получает состояние Отравленный до начала следующего хода дьявола. Пока цель отравлена таким образом, она не может восстанавливать Хиты.' },
      { name: 'Дьявольская глефа', desc: 'Бросок рукопашной атаки: +5, досягаемость 10 фт. Попадание: 8 (1к10 + 3) Рубящего урона. Если у цели нет дьявольской раны — Спасбросок Телосложения Сл 12. Провал: цель теряет 5 (1к10) Хитов в начале каждого своего хода. Рана закрывается спустя 1 минуту, после восстановления Хитов заклинанием или успешной проверки Мудрости (Медицина) Сл 12 в пределах 5 фт.' },
    ],
  },
  {
    ...
    tags: ['Дьявол', 'Законно-Злой', 'Средний'],
    habitat: 'Девять Преисподних',
    cr: '5', xp: '1800', profBonus: '+3',
    ac: 15, initiative: '+3', hp: '110 (13к8 + 52)', speed: '30 фт.',
    stats: [
    ...
    treasures: '',
    traits: [
      { name: 'Шипастая шкура', desc: 'В начале каждого своего хода шипастый дьявол причиняет 5 (1к10) колющего урона всем схватившим его существам.' },
      { name: 'Дьявольское зрение', desc: 'Магическая тьма не мешает тёмному зрению дьявола.' },
      { name: 'Сопротивление магии', desc: 'Дьявол совершает с преимуществом спасброски от заклинаний и прочих магических эффектов.' },
    ],
    actions: [
      { name: 'Мультиатака', desc: 'Дьявол совершает три рукопашные атаки: одну хвостом и две когтями. Альтернативно — дважды использует Метание пламени.' },
      { name: 'Коготь', desc: 'Рукопашная атака оружием: +6 к попаданию, досягаемость 5 фт., одна цель. Попадание: 6 (1к6 + 3) колющего урона.' },
      { name: 'Хвост', desc: 'Рукопашная атака оружием: +6 к попаданию, досягаемость 5 фт., одна цель. Попадание: 10 (2к6 + 3) колющего урона.' },
      { name: 'Метание пламени', desc: 'Дальнобойная атака заклинанием: +5 к попаданию, дистанция 150 фт., одна цель. Попадание: 10 (3к6) урона огнём. Если цель — горючий предмет, которого никто не несёт и не носит, она загорается.' },
    ],
    description: 'Существа с необузданными жадностью и желаниями, шипастые дьяволы служат охранниками для более сильных обитателей Девяти Преисподних и их сокровищниц. Шипастые дьяволы известны своей бдительностью — их трудно застать врасплох, и они относятся к своим обязанностям без скуки и рассеянности.',
  },
  {
    name: 'Цепной дьявол',
    subtitle: 'Китон',
    ...
    description: 'Это зловещее исчадие носит цепи как одежду. Дьяволы цепей выступают в роли тюремщиков-садистов, наслаждающихся болью и живущих только ради того, чтобы эту боль причинять.',
  },
  {
    ...
    description: 'Движимые ненавистью, вожделением и завистью, костяные дьяволы служат жестокими надсмотрщиками Девяти Преисподних. Они принуждают более слабых дьяволов к работе, получая особое наслаждение, когда их соперников понижают в ранге. Выглядят как гуманоидная оболочка с высохшей кожей на скелетном остове — с черепом вместо головы и хвостом скорпиона.',
  },
    ],
  },
  {
...
// ─── Иконки глав бестиария ───────────────────────────────────────────────────

const ChapterIcon = ({ icon, size = 14, className = '' }: { icon: string; size?: number; className?: string }) => {
  if (icon === 'pentagram') {
    // Правильная пятиконечная звезда (пентаграмма)
    const r = size / 2;
    const cx = r, cy = r;
    ...
    return (
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className={className} fill="none" stroke="currentColor" strokeWidth="1.2">
        <path d={path} />
      </svg>
    );
  }
  if (icon === 'abyss') {
    // Искажённая перевёрнутая звезда с расплывающимися лучами — символ Бездны
    const s = size;
    return (
      <svg width={s} height={s} viewBox="0 0 20 20" className={className} fill="currentColor">
        <path d="M10 18.5 L8.5 13.2 L3.5 16.2 L7.2 11.8 L2 10.5 L7.2 9.8 L4 5.5 L8.8 8.2 L10 3 L11.8 8.5 L16 5 L13.2 9.8 L18.5 10.5 L13 12 L16.5 16.5 L11.5 13.5 Z" opacity="0.9"/>
        <path d="M10 20 L9 14 L4 18 L8 12.5 L1.5 12 L8 10 L3 5 L9 9 L10 2 L11.5 9.5 L17 5.5 L13 10 L19 12 L13 13 L17.5 18.5 L11 14 Z" opacity="0.4"/>
      </svg>
    );
  }
  return null;
      ...
      <div className="mt-14 max-w-4xl mx-auto">
        ...
        <div className="flex gap-2 mb-4">
          {BESTIARY_CHAPTERS.map((ch, i) => (
            ...
            >
              <ChapterIcon icon={ch.icon} size={14} />
              {ch.chapter}
            </button>
          ))}
        </div>
        ...
        >
          ...
          <div className="absolute top-3 left-8 z-10 hidden md:flex items-center gap-1.5">
            <ChapterIcon icon={chapter.icon} size={11} className="text-gold/40" />
